		...
		<el-form ref="dataFormRef" :model="dataForm" :rules="dataRules" label-width="120px"
			...
			<el-row>
				<el-col :span="12">
					...
					<el-form-item prop="orgId" label="店铺">
						<!-- <el-input v-model="dataForm.username" placeholder="用户名"></el-input> -->
						<el-select v-model="dataForm.orgId" placeholder="店铺" style="width: 100%" disabled>
							<el-option v-for="shop in shopList" :key="shop.id" :label="shop.name"
...
import { useShopListApi } from '@/api/egg/shop'
...
const init = (id?: number) => {
	...
	if (id) {
		...
		if(user.superAdmin===1){
			ElNotification({
				title: '提示',
				message: '总管理员请到系统管理->用户管理添加',
				type: 'warning',
			})
			router.push('/sys/user/index')
		}
		dataForm.orgId = user.orgId 
	}
